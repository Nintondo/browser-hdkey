...
class HDKey {
    ...
    deriveChild(index) {
        ...
        if (this.privateKey) {
            ...
            try {
                hd.setPublicKey(Buffer.from(secp256k1.publicKeyTweakAdd(this._publicKey, IL, true)));
            }
            catch (err) {
                return this.deriveChild(index + 1);
            }
    ...
    sign(hash) {
        return Buffer.from(secp256k1.ecdsaSign(Uint8Array.from(hash), Uint8Array.from(this._privateKey)).signature);
    }
    verify(hash, signature) {
        return secp256k1.ecdsaVerify(Uint8Array.from(signature), Uint8Array.from(hash), Uint8Array.from(this._publicKey));
    }
    wipePrivateData() {
        if (this._privateKey)
